...
func (uu *UserUpdate) Save(ctx context.Context) (int, error) {
	...
	if len(uu.hooks) == 0 {
		...
		for i := len(uu.hooks) - 1; i >= 0; i-- {
			if uu.hooks[i] == nil {
				return 0, fmt.Errorf("ten: uninitialized hook (forgotten import ten/runtime?)")
			}
			mut = uu.hooks[i](mut)
		}
...
func (uu *UserUpdate) check() error {
	if v, ok := uu.mutation.Age(); ok {
		if err := user.AgeValidator(v); err != nil {
			return &ValidationError{Name: "age", err: fmt.Errorf(`ten: validator failed for field "User.age": %w`, err)}
		}
	}
	return nil
}
...
func (uuo *UserUpdateOne) Save(ctx context.Context) (*User, error) {
	...
	if len(uuo.hooks) == 0 {
		...
		for i := len(uuo.hooks) - 1; i >= 0; i-- {
			if uuo.hooks[i] == nil {
				return nil, fmt.Errorf("ten: uninitialized hook (forgotten import ten/runtime?)")
			}
			mut = uuo.hooks[i](mut)
		}
...
func (uuo *UserUpdateOne) sqlSave(ctx context.Context) (_node *User, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ten: missing "User.id" for update`)}
	}
	...
	if fields := uuo.fields; len(fields) > 0 {
		...
		for _, f := range fields {
			if !user.ValidColumn(f) {
				return nil, &ValidationError{Name: f, err: fmt.Errorf("ten: invalid field %q for query", f)}
			}
			if f != user.FieldID {
