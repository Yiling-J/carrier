...
func (uq *UserQuery) QueryRecipes() *RecipeQuery {
	query := &RecipeQuery{config: uq.config}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withRecipes: uq.withRecipes.Clone(),
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withRecipes; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Recipes = []*Recipe{}
		}
		query.withFKs = true
		query.Where(predicate.Recipe(func(s *sql.Selector) {
			s.Where(sql.InValues(user.RecipesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_recipes
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_recipes" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_recipes" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Recipes = append(node.Edges.Recipes, n)
		}
	}
